...
client.on("guildMemberAdd", (member) => {
    onGuildMemberAdd.run(member);
});

client.on("guildMemberRemove", (member) => {
    onGuildMemberRemove.run(member);
});

client.on("guildCreate", guild => {
    onGuildCreate.run(guild, client);
});

client.on('ready', () => {
    onReady.run(client, prefix);
});

client.on('message', message => {
    onMessage.run(client, message, prefix, Discord);
    return;
});
